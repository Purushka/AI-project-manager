...
在分解到达检查点层级（L2, L4, L6, L9）时，对当前所有已完成节点进行聚类分析，发现潜在的共享组件和复用机会。

## 工作流程

1. **收集标签**：从数据库中提取所有目标层级节点的超空间向量标签
2. **结构化聚类**：基于 tags 表做集合运算
   - 相同 domain 标签的节点
   - 相同 entities 的节点
   - 相似 api_shape 的节点
   - 相同 patterns 的节点
3. **语义聚类**：基于 ChromaDB 中的 summary embedding 做 KMeans/DBSCAN 聚类
